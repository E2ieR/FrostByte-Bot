...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const amount = raw.toLowerCase() === 'all' ? user.coins : parseInt(raw);
            if (isNaN(amount) || amount <= 0) return interaction.reply({ content: '❌ จำนวนไม่ถูกต้อง', ephemeral: true });
            if (user.coins < amount) return interaction.reply({ content: `❌ เงินในกระเป๋าไม่พอ (มี ${user.coins.toLocaleString()})`, ephemeral: true });
            if (config.maxBankBalance > 0 && user.bank + amount > config.maxBankBalance)
                return interaction.reply({ content: `❌ เกินวงเงินสูงสุดในธนาคาร (${config.maxBankBalance.toLocaleString()})`, ephemeral: true });
            user.coins -= amount;
            user.bank  += amount;
            await user.save();
            const e = new EmbedBuilder().setColor(0x57F287).setTitle('🏦 ฝากเงินสำเร็จ')
                .addFields(
                    { name: 'ฝากเข้า', value: `**${amount.toLocaleString()}** ${config.currencyEmoji}`, inline: true },
